...
/**
 * Path: Sites / site-name / Menu items <br>
 * Page object for site's menu items. This page contains a table with menu
 * items: title columns, type columns...
 * 
 */
public class SiteMenuItemsTablePage extends AbstractAdminConsolePage
{
	...
	private String ITEM_XPATH = "//td[contains(@class,'browsetablecell')]/span[text()='%s']";
	@FindBy(name = "cmdNew")
	...
	/**
	 * Clicks by 'menu item' and edit it.
	 * @param itemToUpdate
	 * @param menuItem
	 */
	public void doEditMenuItem(MenuItem itemToUpdate, MenuItem menuItem)
	{
		// find a menu item and click by it
		String siteNameXpath = String.format(ITEM_XPATH, itemToUpdate.getDisplayName().toLowerCase());
		List<WebElement> elems = getSession().getDriver().findElements(By.xpath(siteNameXpath));
		if (elems.size() == 0)
		{
			throw new TestFrameworkException("menu item was not found, name:" + itemToUpdate.getDisplayName());
		}
		elems.get(0).click();
		if (menuItem instanceof PageMenuItem)
		{
			...
			wizard.doEdit((PageMenuItem) itemToUpdate, (PageMenuItem) menuItem);
		}
