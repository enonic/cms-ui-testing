 ...
 */
public class ContentWithTinyMCEWizard
    extends AbstractAddContentWizard<ContentWithEditorInfo>
{
    ...
    private final String INSERT_IMAGE_BUTTON_XPATH =
        "//td[contains(@class,'mceToolbar')]//a[@role='button' and contains(@class,'mce_cmsimage')]";

    private final String INSERT_TABLE_BUTTON_XPATH =
        "//td[contains(@class,'mceToolbar')]//a[@role='button' and contains(@class,'mce_table')]";

    private final String INSERT_SPECIAL_SYMBOLS_BUTTON_XPATH =
        "//td[contains(@class,'mceToolbar')]//a[@role='button' and contains(@class,'mce_charmap')]";

    private final String INSERT_DELETION_BUTTON_XPATH =
        "//td[contains(@class,'mceToolbar')]//a[@role='button' and contains(@class,'mce_del')]";

    private final String INSERT_INSERTION_BUTTON_XPATH =
        "//td[contains(@class,'mceToolbar')]//a[@role='button' and contains(@class,'mce_ins')]";

    private final String INSERT_BLOCKQUOTE_BUTTON_XPATH =
        "//td[contains(@class,'mceToolbar')]//a[@role='button' and contains(@class,'mce_blockquote')]";

    private final String INSERT_ABBREVIATION_BUTTON_XPATH =
        "//td[contains(@class,'mceToolbar')]//a[@role='button' and contains(@class,'mce_abbr')]";

    private final String INSERT_ACRONYM_BUTTON_XPATH =
        "//td[contains(@class,'mceToolbar')]//a[@role='button' and contains(@class,'mce_acronym')]";

    private final String INSERT_CITATION_BUTTON_XPATH =
        "//td[contains(@class,'mceToolbar')]//a[@role='button' and contains(@class,' mce_cite')]";

    private final String INSERT_SUPERSCRIPT_BUTTON_XPATH =
        "//td[contains(@class,'mceToolbar')]//a[@role='button' and contains(@class,'mce_sup')]";

    private final String INSERT_SUBSCRIPT_BUTTON_XPATH =
        "//td[contains(@class,'mceToolbar')]//a[@role='button' and contains(@class,'mce_sub')]";

    private final String INSERT_BULLIST_BUTTON_XPATH =
        "//td[contains(@class,'mceToolbar')]//a[@role='button' and contains(@class,'mce_bullist')]";

    private final String INSERT_NUMLIST_BUTTON_XPATH =
        "//td[contains(@class,'mceToolbar')]//a[@role='button' and contains(@class,'mce_numlist')]";

    private final String INCREASE_INDENT_BUTTON_XPATH =
        "//td[contains(@class,'mceToolbar')]//a[@role='button' and contains(@class,'mce_indent')]";

    private final String DECREASE_INDENT_BUTTON_XPATH =
        "//td[contains(@class,'mceToolbar')]//a[@role='button' and contains(@class,'mce_outdent')]";
    ...
    /**
     * Types a string, clicks by 'Increase Indent' button , verify style:
     * padding-left: 30px; click by 'Decrease Indent' after that.
     */
    public void verifyIncreaseDecreaseIndents()
    {
        ...
        if ( !isPresent )
        {
            TestUtils.getInstance().saveScreenshot( getSession(), "err_decrease1" );
            Assert.fail( "actual innerHtml does not contain expected string: " + expectedText );
        }
        ...
        if ( isPresent )
        {
            TestUtils.getInstance().saveScreenshot( getSession(), "err_decrease2" );
    ...
    public void verifyInsertBulletedList()
    {
        ...
        if ( !isPresent )
        {
            TestUtils.getInstance().saveScreenshot( getSession(), "err_bulletedList" );
    ...
    public void verifyInsertNumberedList()
    {
        ...
        if ( !isPresent )
        {
            TestUtils.getInstance().saveScreenshot( getSession(), "err_numblist" );
            Assert.fail( "actual innerHtml does not contain expected strings: " );
        }
    }
    ...
    public void verifyInsertSuperscript()
    {
        ...
        if ( !isPresent )
        {
            TestUtils.getInstance().saveScreenshot( getSession(), "err_superscr" );
            Assert.fail( "actual innerHtml does not contain expected string: " + expectedText );
        }
    }
    ...
    public void doAddCitation()
    {
        ...
        if ( !allWindows.isEmpty() )
        {
            ...
            for ( String windowId : allWindows )
            {
                ...
                try
                {
                    if ( getDriver().switchTo().window( windowId ).getTitle().contains( INSERT_CITATION_POPUP_TITLE ) )
                    {
    ...
    /**
     * Switches to the popup window, selects a character and press "Insert"
     * button.
     *
     * @param character
     */
    ...
    /**
     * Clicks by 'Insert Image' button, add image and verify: image is present
     * on the page.
     */
                ...
                try
                {
                    if ( getDriver().switchTo().window( windowId ).getTitle().equals( SELECT_CONTENT_POPUP_WINDOW_TITLE ) )
                    {
                        for ( int i = 0; i < path.length - 1; i++ )
                        {
                            ...
                            String expanderFolderXpath =
                                String.format( "//td/span[contains(@id,'menuitemText') and contains(.,'%s')]/../../td/a/img", path[i] );
                        ...
                        // select checkbox for content and press the butoon
                        // 'Add'
    ...
    /**
     * Inserts a table and verify: table with defined height and width is
     * present in Editor
     */
            ...
            for ( String windowId : allWindows )
            {
                ...
                try
                {
                    if ( getSession().getDriver().switchTo().window( windowId ).getTitle().equals( INSERT_TABLE_POPUP_TITLE ) )
                    {
                        ...
                        TestUtils.getInstance().clearAndType( getSession(), elems.get( 0 ),
                                                              String.valueOf( tableToInsert.getColumnsNumber() ) );
                        ...
                        TestUtils.getInstance().clearAndType( getSession(), elems.get( 0 ),
                                                              String.valueOf( tableToInsert.getRowsNumber() ) );
                        // 3. insert Width.
                        ...
                        elems.get( 0 ).click();
                        // popup window closed, need to switch to the main
                        // window
    ...
    public void verifyLinkUnlink()
    {
        ...
        doAddLink( linkText );
        // verify:
        // String expectedText = String.format("<a href=\"http://%s\"",
        // linkText);
    ...
    private boolean verifyTextInEditor( String... expected )
    {
        ...
        //System.out.println(" actual text is:  " + actualInnerHtml);
        getLogger().info( " actual text is:  " + actualInnerHtml );
        for ( String s : expected )
        {
            getLogger().info( "should contains:  " + s );
    ...
    public void verifyAnchorInText()
    {
        ...
        // String expectedText =
        // String.format("<p><a class=\"mceItemAnchor\" name=\"%s\"></a></p>",
        // anchorText);
    ...
    public void verifyChangeBackgroundColorText()
    {
        ...
        List<WebElement> elems =
            findElements( By.xpath( "//a[@title='Select Background Color' and descendant::span[@class='mceIconOnly']]" ) );
        elems.get( 0 ).click();
        TestUtils.getInstance().waitAndFind( By.xpath( "//a[@title='Red']" ), getDriver() );
        elems = findElements( By.xpath( "//a[@title='Red']" ) );
        elems.get( 0 ).click();
    ...
    public void verifyTextAlignment( AlignmentText align )
    {
        ...
        if ( !result )
        {
            TestUtils.getInstance().saveScreenshot( getSession(), "err_alignment" );
    ...
    public void waituntilPageLoaded( long timeout )
    {
        new WebDriverWait( getDriver(), timeout ).until(
            ExpectedConditions.visibilityOfElementLocated( By.xpath( GENERAL_TAB_LINK_XPATH ) ) );
