...
public class ImportContentWithBlockGroup
    extends BaseTest
{
    /** configurations of Content Types
     * mode=xml; format="dd-MM-yyyy"; Added "block name='Events' "
     */
    private static final String PERSON_EVENTS_CFG = "test-data/contenttype/person-block.xml";

    /**
     * mode =xml; Added purge="true" for EVENTS
     */
    private static final String PERSON_BLOCK_EVENTS_PURGE_CFG = "test-data/contenttype/person-events-purge.xml";

    private static final String PERSON_CONTENT_PURGE_ARCHIVE_CFG = "test-data/contenttype/person-content-purge-archive.xml";

    private static final String PERSON_CONTENT_PURGE_DELETE_CFG = "test-data/contenttype/person-content-purge-delete.xml";

    /**
     * resources to IMPORT
     */
    private static final String IMPORT_PERSONS_BLOCK_GROUP_XML_FILE = "test-data/contentimport/persons-block-groups.xml";

    private static final String IMPORT_PERSONS_BLOCK_GROUP_UPDATE_XML_FILE = "test-data/contentimport/persons-block-groups-update.xml";

    private static final String IMPORT_PERSONS_BLOCK_GROUP_PURGE_UPDATE_XML_FILE =
        "test-data/contentimport/persons-block-groups-purge-update.xml";

    /**
     * delete one person from XML formatted source
     */
    private static final String IMPORT_PERSONS_CONTENT_PURGE_XML_FILE = "test-data/contentimport/person-content-purge.xml";
     ...
     */
    private static final String PERSON_CTYPE_NAME = "Person-events";

    /**
     * key for saving a Category in the Session
     */
    private static final String IMPORT_CATEGORY_BLOCK_GROUPS_KEY = "category_event_block_groups";

    /**
     * key for saving a Content Type  in the Session
     */
    private static final String PERSON_CTYPE_KEY = "person-events-ctype";


    private static final String EVENT_VALUE_PURGE_TEST = "04-07-1895";

    private static final String PERSON_EVENT_PURGE_TEST = "Fridtjof Nansen";

    private static final String EVENT_NAME_PURGE_TEST = "North pole almost reached";

    private static final String CONTENT_PURGE_PERSON_NAME = "Eva Helene Sars";
    ...
    @Test(dependsOnMethods = "settings",
          description = "Importing into block groups,Mapping input fields of type date, format='dd-MM-yyyy' ")
    public void importingIntoBlockGroups()
    {
        ...
        ContentsTableFrame table =
            contentService.doImportContent( getTestSession(), "person-import-xml", IMPORT_PERSONS_BLOCK_GROUP_XML_FILE, false, 4l,
                                            pathToCategory );
        ...
        Assert.assertTrue( personsFromXML.size() == namesActual.size(),
                           "the number of persons from UI and expected number of persons are not equals!" );
     ...
     * dest="date-of-birth"/>
     */
    @Test(dependsOnMethods = "importingIntoBlockGroups",
          description = "Updating block group entries, sync='person-no'.There are a  new person and a new event in the XML formatted source. ")
    public void updatingBlockGroupEntriesTest()
    {
        logger.info(
            "####  STARTED: Updating block group entries, sync='person-no'. There are a  new person and a new event in the XML formatted source. " );
        logger.info( " Mapping input fields of type date" );
        ContentCategory categoryForImport = (ContentCategory) getTestSession().get( IMPORT_CATEGORY_BLOCK_GROUPS_KEY );
        String[] pathToCategory = new String[]{categoryForImport.getParentNames()[0], categoryForImport.getName()};
        // 1. update content: new event was added for first person and new person added:
        ContentsTableFrame table =
            contentService.doImportContent( getTestSession(), "person-import-xml", IMPORT_PERSONS_BLOCK_GROUP_UPDATE_XML_FILE, false, 4l,
                                            pathToCategory );
        List<String> namesActual = table.getContentNames();
        XmlReader xmlReader = new XmlReader();
        // 2. gets EXPECTED persons with events from the XML
...

    {
        ...
        Assert.assertTrue( eventsBefore.contains( eventsBeforeUpdate ),
                           "before Importing: event with name:" + EVENT_NAME_PURGE_TEST + "should be present on the page" );

        // 3. import and update content: "North pole almost reached" event should be removed for 'Fridtjof Nansen' person
        table =
            contentService.doImportContent( getTestSession(), "person-import-xml", IMPORT_PERSONS_BLOCK_GROUP_PURGE_UPDATE_XML_FILE, false,
                                            4l, pathToCategory );
        logger.info(
            "import of XML formatted source finished. Content was updated: filename " + IMPORT_PERSONS_BLOCK_GROUP_PURGE_UPDATE_XML_FILE );
        // verify that
        List<UserEvent> eventsAfterImport = ImportUtils.getEventsFromUI( getSessionDriver(), table, PERSON_EVENT_PURGE_TEST );
        Assert.assertFalse( eventsAfterImport.contains( eventsBeforeUpdate ),
                            "event with name:" + EVENT_NAME_PURGE_TEST + " still present on the page, but should be removed!" );
...

    {
        ...
        table = contentService.doImportContent( getTestSession(), "person-import-xml", IMPORT_PERSONS_CONTENT_PURGE_XML_FILE, false, 4l,
                                                pathToCategory );
...

    {
        ...
        Assert.assertFalse( isPresentPersonAfterImport,
                            "the person with name: " + CONTENT_PURGE_PERSON_NAME + "should be deleted from the table!" );
