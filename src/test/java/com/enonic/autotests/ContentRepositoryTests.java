    ...
    extends BaseTest
{
    ...
    @Test(dependsOnMethods = "settingsForContentRepository", description = "create new Content Repository with specified Content Type",
          dataProvider = "createContentRepositoryPositive", dataProviderClass = ContentRepositoryProvider.class)
    ...
    @Test(description = "add category(Content Type == 'Files') to the repository, top category have no content type.",
          dependsOnMethods = "createRepositoryTest")
    ...
    @Test(description = "Empty all content from category. Open category, and delete by pushing 'Delete'",
          dependsOnMethods = "addContentToCategoryTest")
    ...
    public void publishContentToOrderedSection()
    {
        ...
        boolean isContentPublished =
            siteService.isContentFromSectionPublished( getTestSession(), section, sectionContent.getDisplayName() );
        Assert.assertTrue( isContentPublished,
                           "content publishing failed, content not present in the section or content is not published!" );
     ...
     */
    @Test(dependsOnMethods = "addContentToSectionTest",
          description = "Remove content from section, verify: content not present in section view")
    public void removeFromSectionTest()
    {
        logger.info( "##### STARTED :add content to section, verify: content visible in section view" );
        SectionMenuItem section = (SectionMenuItem) getTestSession().get( TEST_SECTION_FILE_CTYPE );
        Site site = (Site) getTestSession().get( TEST_SITE );
        SectionContentsTablePage table =
            siteService.removeContentFromSection( getTestSession(), site.getDispalyName(), section.getDisplayName(),
                                                  SECTION_CONTENT_ADD_NAME );
